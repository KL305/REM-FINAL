...
SELECT CAST(SCOPE_IDENTITY() as int);", connection))
        ...
        public UserViewModel GetUserByUsername(string username)
        {
            try
            {
                using (var connection = new SqlConnection(Database.ConnectionString))
                using (var command = new SqlCommand(@"SELECT [UserId], [Username], [Email], [UserRole] 
                    FROM [dbo].[Users] WHERE [Username] = @Username", connection))
                {
                    command.Parameters.Add("@Username", SqlDbType.NVarChar, 100).Value = username;
                    connection.Open();
                    using (var reader = command.ExecuteReader())
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Error in GetUserByUsername: {ex.Message}");
                throw;
            }
            return null;
        }
        ...
        public bool ValidateCurrentPassword(int userId, string currentPassword)
        {
            ...
            {
                using (var connection = new SqlConnection(Database.ConnectionString))
                using (var command = new SqlCommand(@"SELECT COUNT(1) FROM [dbo].[Users] 
                    WHERE [UserId] = @UserId AND [Password] = @Password", connection))
                {
                    command.Parameters.Add("@UserId", SqlDbType.Int).Value = userId;
                    command.Parameters.Add("@Password", SqlDbType.NVarChar, 256).Value = currentPassword;
                    connection.Open();
                    var count = (int)command.ExecuteScalar();
                    return count > 0;
                }
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Error in ValidateCurrentPassword: {ex.Message}");
                throw;
            }
        }

        public bool UpdateUserCredentials(int userId, string username, string email, string newPassword = null, string currentPassword = null)
        {
            // For self-update, validate current password if changing password
            ...
            try
            {
                using (var connection = new SqlConnection(Database.ConnectionString))
                {
                    ...
                    using (var command = new SqlCommand(query, connection))
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(newPassword))
                        {
                            command.Parameters.Add("@Password", SqlDbType.NVarChar, 256).Value = newPassword;
                        }
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Error in UpdateUserCredentials: {ex.Message}");
                throw;
            }
        }
    }
